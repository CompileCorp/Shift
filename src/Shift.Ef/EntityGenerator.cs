...
public class EntityGenerator
{
    ...
    private void GenerateProperty(StringBuilder sb, FieldModel field)
    {
        ...
        // Add Column attribute with type information if needed
        if (field.Type.ToLower() == "decimal" && field.Precision.HasValue)
        {
            if (field.Scale.HasValue)
            {
                columnAttributes.Add($"[Column(\"{field.Name}\", TypeName = \"decimal({field.Precision},{field.Scale})\")]");
            }
            else
            {
                columnAttributes.Add($"[Column(\"{field.Name}\", TypeName = \"decimal({field.Precision})\")]");
            }
        }
        else
        {
